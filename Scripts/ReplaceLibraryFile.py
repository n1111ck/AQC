...
def main():
    propsFile = open(work_path() + "Scripts\\UnrealEngineProps.json", "r")
    ...
    if len(platform_options) > 0:
        print("[ENTRADA] Escolha a plataforma da biblioteca a ser copiada")
        for id, platform in enumerate(platform_options):
            print(f"\t{id + 1} - {platform}")
        resposta = int(input("Opcao: ")) - 1
